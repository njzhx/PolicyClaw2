...
headers = {
    ...
    'Referer': 'https://doc.jiangsu.gov.cn/col/col80617/index.html?uid=310478&pageNum=1',
    'Accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,*/*;q=0.8'
}

TARGET_URL = "https://doc.jiangsu.gov.cn/col/col80617/index.html?uid=310478&pageNum=1"
    ...
    try:
        ...
        for article in article_list:
            try:
                ...
                # 如果还是没有日期，尝试从标题中提取公告日期
                if not pub_at:
                    # 匹配 "2026年第11号" 这样的格式
                    notice_date_pattern = r'(20\d{2})年第(\d+)号'
                    notice_date_match = re.search(notice_date_pattern, title)
                    if notice_date_match:
                        year = notice_date_match.group(1)
                        # 假设公告发布在当年，使用1月1日作为默认日期
                        try:
                            pub_at = datetime(int(year), 1, 1).date()
                        except Exception:
                            pass
                    else:
                        # 匹配 "2025第62号" 这样的格式
                        notice_date_pattern2 = r'(20\d{2})第(\d+)号'
                        notice_date_match2 = re.search(notice_date_pattern2, title)
                        if notice_date_match2:
                            year = notice_date_match2.group(1)
                            try:
                                pub_at = datetime(int(year), 1, 1).date()
                            except Exception:
                                pass
        ...
        print(f"✅ 江苏省商务厅政策及公告爬虫：成功抓取 {len(policies)} 条目标日期窗口数据")
...
def save_to_supabase(data_list):
    try:
        ...
        return save_to_policy(data_list, "江苏省商务厅_政策及公告")
    except Exception:
        return data_list
