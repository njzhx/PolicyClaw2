...
def scrape_data():
    ...
    while True:
        if page == 1:
            ...
            list_url = f"https://ggzy.suzhou.gov.cn/zcwj/moreinfo.html?categoryNum=040&pageIndex={page}"
        ...
        soup = BeautifulSoup(response.content, "html.parser")
        rows = soup.select("tr.ewb-trade-tr")
        if not rows:
            break
        ...
        for row in rows:
            try:
                tds = row.select("td.ewb-trade-td")
                if len(tds) < 3:
                    metrics.invalid_item_count += 1
                    continue
                link_td = tds[1]
                link = link_td.select_one("a")
                if not link:
                    metrics.invalid_item_count += 1
                    continue
