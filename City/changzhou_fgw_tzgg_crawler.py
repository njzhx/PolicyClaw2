...
站点：常州市发改委 通知公告（gova/tzgg.php 表格）
...
TARGET_URL = "https://www.changzhou.gov.cn/gova/tzgg.php?catname=fgw_02_10"
SOURCE_NAME = "常州市发展和改革委员会_通知公告"
CATEGORY = "常州"
...
def _parse_list_page(html):
    """解析 wjk-list 表格，返回 [{title, href, date}]。"""
    soup = BeautifulSoup(html, "html.parser")
    nodes = []
    for row in soup.select("table.wjk-list tr"):
...
def scrape_data():
    ...
    for page_index in range(1, MAX_PAGES + 1):
        if page_index == 1:
            ...
            page_url = f"{TARGET_URL}{separator}page={page_index}"
