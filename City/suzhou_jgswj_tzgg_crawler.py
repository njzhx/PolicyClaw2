...
TARGET_URL = "https://jgswj.suzhou.gov.cn/szjgswj/tzgg/nav_list_2.shtml"
SOURCE_NAME = "苏州市机关事务管理局_通知公告"
CATEGORY = "苏州"
MAX_PAGES = 200
...
def _get_page_url(page: int) -> str:
    base = TARGET_URL.replace("/nav_list_2.shtml", "")
    if page <= 1:
        return f"{base}/nav_list_2.shtml"
    return f"{base}/nav_list_{page + 1}.shtml"
...
def scrape_data():
    ...
    try:
        for page in range(1, MAX_PAGES + 1):
            ...
            soup = BeautifulSoup(response.content, "html.parser")
            nodes = soup.select(".page-infolist ul > li")
            if not nodes:
                break
            ...
            for node in nodes:
                try:
                    ...
                    date_elem = node.select_one("span.time") or node.select_one("span")
