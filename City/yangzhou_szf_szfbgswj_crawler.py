...
TARGET_URL = "https://www.yangzhou.gov.cn/zfxxgk/fdgkzdnr/zfwj/szfbgswj/index.html"
SOURCE_NAME = "扬州市人民政府_市政府办公室文件"
CATEGORY = "扬州"
...
def _extract_content(article_url, metrics):
    try:
        ...
        for sel in [
            "div.bt-content.zoom.jpaas-ai-content",
            "div.content#zoom",
            ".bt-content",
            "#zoom",
            ".article-content",
            ...
            if elem:
                ...
                text = elem.get_text("\n", strip=True)
                if text:
                    assist = elem.select_one(".bt-art-assist-tool")
                    if assist:
                        assist.decompose()
                        text = elem.get_text("\n", strip=True)
                    return text
