"""
徐州市科学技术局_政策文件爬虫
目标栏目：https://kjj.xz.gov.cn/dynamic/zwgk/govInfoPub.html?categorynum=003006
列表接口：https://kjj.xz.gov.cn/EWB-FRONT/rest/lightfrontaction/getgovinfolist
"""
import html
import json
import re
from urllib.parse import parse_qs, urljoin, unquote, urlsplit, urlunsplit
...
TARGET_URL = "https://kjj.xz.gov.cn/dynamic/zwgk/govInfoPub.html?categorynum=003006"
SOURCE_NAME = "徐州市科学技术局_政策文件"
CATEGORY = "徐州"
BASE_URL = "https://kjj.xz.gov.cn/"

LIST_API_URL = "https://kjj.xz.gov.cn/EWB-FRONT/rest/lightfrontaction/getgovinfolist"
SITE_GUID = "83bc469c-9ee0-4d91-ab7b-30d19a8278be"
CATEGORY_NUM = "003006"
TARGET_CATEGORY_NAME = "政策文件"
PAGE_SIZE = 20
MAX_PAGES = 100
CRAWLER_FIX_VERSION = "2026-08-10-r3"
...
_RELATED_KEYWORDS = ("解读", "图解", "相关阅读", "政策解读", "关联阅读", "相关推荐", "一图读懂")
...
def _has_attachment_ext(value):
    if not value:
        return False
    decoded = unquote(html.unescape(str(value))).strip().lower()
    parsed = urlsplit(decoded)

    if any(parsed.path.endswith(ext) for ext in _ATTACHMENT_EXTS):
        return True

    query = parse_qs(parsed.query, keep_blank_values=True)
    for values in query.values():
        for item in values:
            item_decoded = unquote(html.unescape(item)).strip().lower()
            if any(item_decoded.endswith(ext) for ext in _ATTACHMENT_EXTS):
                return True

    return any(decoded.endswith(ext) for ext in _ATTACHMENT_EXTS)
...
def _is_attachment_link(a_tag):
    ...
    href = html.unescape((a_tag.get("href") or "").strip())
    if not href:
        return False
    ...
    if _has_attachment_ext(href):
        return True

    visible_text = _clean_title(a_tag.get_text(" ", strip=True))
    return _has_attachment_ext(visible_text)

def _remove_noise_elements(elem):
    for tag_name in ("script", "style", "noscript", "template", "iframe"):
        for node in elem.find_all(tag_name):
            node.decompose()
    noise_texts = {"关闭本页", "打印本页", "返回顶部", "网站地图", "扫一扫"}
...
def _truncate_functional_tail(text):
    if not text:
        return ""
    stop_markers = ("相关阅读", "分享开始", "关联阅读", "相关推荐")
    exact_noise = {"关闭本页", "打印本页", "返回顶部", "网站地图", "扫一扫", "end"}
    kept = []
...
def _attachment_name_from_url(url):
    ...
    for key in ("filename", "file", "name", "path", "url"):
        for value in query.get(key, []):
            candidate = unquote(html.unescape(value)).strip()
            if _has_attachment_ext(candidate):
    ...
    return path_name if _has_attachment_ext(path_name) else ""
...
def _extract_attachments(soup, article_url):
    ...
    for a_tag in soup.find_all("a", href=True):
        ...
        if not _has_attachment_ext(name):
            url_name = _attachment_name_from_url(absolute_url)
            if url_name:
                if name and "." in url_name:
    ...
    # 兜底：有些政府站点把附件URL放在脚本或自定义属性中。
    markup = html.unescape(str(soup))
    pattern = re.compile(
        r"""(?ix)(https?://[^\s"'<>]+|/[^\s"'<>]+)"""
    )
    for candidate in pattern.findall(markup):
        candidate = candidate.rstrip("),;")
        if not _has_attachment_ext(candidate):
            continue

        absolute_url = _normalize_url(candidate, article_url)
        if not absolute_url or absolute_url in seen_urls:
            continue

        name = _attachment_name_from_url(absolute_url)
        if not name:
            continue
...
def _extract_attachments_from_markup(markup, article_url):
    """兜底提取脚本/非标准HTML中的真实附件URL。"""
    if not markup:
        return []
    ...
    patterns = (
        r'(?i)(?:https?://kxjst\.jiangsu\.gov\.cn)?/module/download/downfile\.jsp\?[^"\'<>\\\s]*?filename=[^"\'<>\\\s&]+(?:\.pdf|\.docx?|\.xlsx?|\.zip|\.rar|\.wps)',
        r'(?i)(?:https?://[^"\'<>\\\s]+|/[^"\'<>\\\s]+)\.(?:pdf|docx?|xlsx?|zip|rar|wps)(?:\?[^"\'<>\\\s]*)?',
    )
    ...
    for candidate in candidates:
        ...
        if not absolute_url or absolute_url in seen_urls:
            continue
        if not _has_attachment_ext(absolute_url):
            continue
        ...
        if pos >= 0:
            context = normalized[max(0, pos - 400): pos + len(candidate) + 400]
            context_text = BeautifulSoup(context, "html.parser").get_text(" ", strip=True)
            if "点击正文.pdf" in context_text:
                name = "点击正文.pdf"
            elif "点击正文" in context_text and name.lower().endswith(".pdf"):
                name = "点击正文.pdf"
        ...
        seen_urls.add(absolute_url)
        attachments.append((_strip_cjk_inner_ws(name), absolute_url))
...
def _is_error_page(soup):
    ...
    if title_tag:
        ...
        if "您访问的页面不存在" in title_text:
            return True
        if "页面未找到" in title_text:
            return True
        if "error" in title_text.lower() and ("404" in title_text or "not found" in title_text.lower()):
            return True
    return False


def _has_valid_content(soup, article_url=""):
    """检查页面是否有有效内容（正文或真实附件）。"""
    body_text = soup.get_text(" ", strip=True)
    if len(body_text) >= 100:
        return True
    return bool(_extract_attachments(soup, article_url))

def _decode_html_response(response):
    ...
    for encoding in (response.apparent_encoding, response.encoding):
        if not encoding:
            continue
        try:
...
def _extract_content(session, article_url, metrics, title=""):
    try:
        ...
        if parsed_url.hostname == "kxjst.jiangsu.gov.cn" and parsed_url.scheme == "http":
        ...
        fetch_parsed = urlsplit(fetch_url)
        referer_base = f"{fetch_parsed.scheme}://{fetch_parsed.netloc}/"
        ...
        content_str = _decode_html_response(response)
        soup = BeautifulSoup(content_str, "html.parser")
        detail_base_url = response.url or fetch_url
        attachments = _extract_attachments(soup, detail_base_url)
        if not attachments:
            attachments = _extract_attachments_from_markup(content_str, detail_base_url)

        if _is_error_page(soup) and not (
            _has_valid_content(soup, detail_base_url) or attachments
        ):
        ...
        text_body = _extract_text_preserve_blocks(content_elem)
        text_body = _clean_content(text_body)
        text_body = _truncate_functional_tail(text_body)

        if attachments:
            text_body = _remove_duplicate_attachment_names(text_body, attachments)

        images = _extract_images(content_elem, detail_base_url)
    ...
    try:
        while page_index < MAX_PAGES:
            ...
            for record in records:
                ...
                if categoryname and _clean_title(categoryname) != TARGET_CATEGORY_NAME:
                    metrics.invalid_item_count += 1
                    continue

                # 绝对URL保持原样，相对URL使用BASE_URL拼接
