...
TARGET_URL = "https://czju.suzhou.gov.cn/czj/cszc/list.shtml"
SOURCE_NAME = "苏州市财政局_财税政策"
CATEGORY = "苏州"
MAX_PAGES = 200
...
def _get_page_url(page: int) -> str:
    if page <= 1:
        return TARGET_URL
    base = TARGET_URL.replace("/list.shtml", "")
    return f"{base}/list_{page}.shtml"
            ...
            for node in nodes:
                try:
                    ...
                    if not link:
                        ...
                        continue
                    # 标题优先取 title 属性，避免 a 内隐藏的 .gjdi ID 混入
