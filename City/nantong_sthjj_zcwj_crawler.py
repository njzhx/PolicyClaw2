"""南通市生态环境局_政策文件 爬虫（栏目聚合页：汇总其下各子栏目）"""
...
TARGET_URL = "https://sthjj.nantong.gov.cn/ntshbj/zcwj/zcwj.html"
SOURCE_NAME = "南通市生态环境局_政策文件"
CATEGORY = "南通"
...
def _fetch_group(session, list_url, column_id, start_record, metrics):
    """调用 getMessage.do 接口抓取一组（30 条）记录，返回 li 节点列表。"""
    api_url = urljoin(list_url, "/truecms/messageController/getMessage.do")
...
def _scrape_sub_column(session, list_url, target_from, target_to,
                       seen_urls, policies, latest_items, metrics):
    """按标准 truecms 流程抓取一个子栏目列表页（含接口翻页）。"""
    try:
        response = session.get(list_url, timeout=LIST_TIMEOUT)
        response.raise_for_status()
        html = response.content.decode("utf-8", errors="replace")
    except Exception as exc:
        metrics.errors.append(f"子栏目列表页抓取失败: {list_url} - {exc}")
        return
    ...
    while True:
        ...
        if total_record and start_record > total_record:
            break
        lis = _fetch_group(session, list_url, column_id, start_record, metrics)
        if lis is None:
            break
...
def scrape_data():
    ...
    try:
        ...
        metrics.errors.append(f"聚合页抓取失败: {exc}")
        return policies, latest_items, metrics

    soup = BeautifulSoup(html, "html.parser")
    sub_urls = []
    for item in soup.select(".container-main-item"):
        title_link = item.select_one(".container-main-title a")
    ...
    if not sub_urls:
        metrics.errors.append("聚合页未发现任何子栏目链接")

    for sub_url in sub_urls:
        _scrape_sub_column(
            session, sub_url, target_from, target_to,
            seen_urls, policies, latest_items, metrics,
        )
