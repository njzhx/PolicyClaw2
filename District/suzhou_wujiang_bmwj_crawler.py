...
def scrape_data():
    ...
    try:
        ...
        departments = []
        for link in soup.select('a[href*="xxgkbuml.shtml"]'):
            query = parse_qs(urlsplit(link["href"]).query)
            values = tuple(query.get(key, [""])[0] for key in ('channel_id', 'deptid'))
    ...
    seen = set()
    api = urljoin(TARGET_URL, '/appsearch/wjapi/list')
    for department in departments:
        ...
        for page in range(1, 501):
            ...
            if deadline and time.time() + 35 >= deadline:
                ...
                break
            params = dict(zip(('channel_id', 'deptid'), department))
            params.update(pageSize=20, currentPage=page)
            try:
