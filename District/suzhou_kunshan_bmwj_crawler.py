"""
苏州市昆山市_部门发文爬虫
目标栏目：http://www.ks.gov.cn/kss/qzbmxxgk/ksxxgkml_ptlink.shtml
说明：目录页提取部门 deptid，访问部门页提取 pid，调用 /appsearch/kssapi/list 接口
"""
import re
...
TARGET_URL = "http://www.ks.gov.cn/kss/qzbmxxgk/ksxxgkml_ptlink.shtml"
SOURCE_NAME = "苏州市昆山市_部门发文"
CATEGORY = "苏州_昆山市"
...
def scrape_data():
    ...
    try:
        ...
        dept_links = []
        for a in soup.select("a[href*='bmxxgkml.shtml']"):
            href = a.get("href") or ""
            if "deptid=" in href:
        ...
        for dept_url in dept_links:
            ...
            if not pid:
                continue
            api_url = f"http://www.ks.gov.cn/appsearch/kssapi/list?pid={pid}&channel_id=&pageSize=15&currentPage=1"
            try:
