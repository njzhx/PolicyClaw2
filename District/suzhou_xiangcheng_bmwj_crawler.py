...
def scrape_data():
    ...
    try:
        ...
        departments = []
        for link in soup.select('a[href*="bmxxgkml_list_all.shtml"]'):
            query = parse_qs(urlsplit(link["href"]).query)
            values = tuple(query.get(key, [""])[0] for key in ('pid',))
            if all(values) and values not in departments:
                departments.append(values)
        if not departments:
    ...
    seen = set()
    api = urljoin(TARGET_URL, '/appsearch/xcapi/listBm')
        ...
        for page in range(1, 501):
            ...
            for item in items:
                try:
                    ...
                    pub_at = parse_date(item.get('qw_xcscrq'))
