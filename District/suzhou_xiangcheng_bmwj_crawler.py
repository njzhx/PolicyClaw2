"""
苏州市相城区_部门发文爬虫
目标栏目：http://www.szxc.gov.cn/szxcrmzf/dfbmlj/xxgkml_ptlink.shtml
说明：目录页提取各部门 pid，调用 /appsearch/xcapi/listBm 接口抓取
"""
...
TARGET_URL = "http://www.szxc.gov.cn/szxcrmzf/dfbmlj/xxgkml_ptlink.shtml"
SOURCE_NAME = "苏州市相城区_部门发文"
CATEGORY = "苏州_相城区"
...
def scrape_data():
    ...
    try:
        ...
        # 目录页提取部门 pid
        dept_pids = []
        for a in soup.select("a[href*='bmxxgkml_list_all.shtml']"):
            href = a.get("href") or ""
            import re as _re
            m = _re.search(r"pid=([a-f0-9]{32})", href)
            if m:
                dept_pids.append(m.group(1))
        dept_pids = list(dict.fromkeys(dept_pids))
        metrics.raw_item_count = len(dept_pids)
        ...
        for pid in dept_pids:
            api_url = (
                f"http://www.szxc.gov.cn/appsearch/xcapi/listBm"
                f"?pid={pid}&pageSize=15&currentPage=1"
            )
            try:
