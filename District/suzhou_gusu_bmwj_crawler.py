...
def _extract_content(session, article_url, metrics):
    try:
        ...
        for selector in ('.TRS_UEDITOR', '.article-content', '#UCAP-CONTENT', '.TRS_Editor', '#zoom', '.Custom_UnionStyle', '.article', '.content'):
            ...
            if has_media:
                break
        if soup.select_one(".article-attachments a[href]"):
            media_only = True
        if media_only:
...
def scrape_data():
    ...
    try:
        ...
        departments = []
        for link in soup.select('a[href*="xxgk_list.shtml"]'):
            query = parse_qs(urlsplit(link["href"]).query)
            values = tuple(query.get(key, [""])[0] for key in ('deptcode',))
    ...
    for department in departments:
        ...
        for page in range(1, 501):
            ...
            if deadline and time.time() + 35 >= deadline:
                ...
                break
            params = dict(zip(('deptcode',), department))
            params.update(pagesize=20, currpage=page)
            try:
