...
def _extract_content(session, article_url, metrics):
    try:
        ...
        media_only = False
        for selector in ('#gov_view', '.TRS_UEDITOR', '.article-content', '#UCAP-CONTENT', '.TRS_Editor', '#zoom', '.Custom_UnionStyle', '.article', '.content'):
...
def scrape_data():
    ...
    seen = set()
    try:
        response = session.post(urljoin(TARGET_URL, "/articleCommonController/showDepartment.do"),
                                data={"rootId": RDEPTID, "type": 5}, headers=API_HEADERS, timeout=30)
        response.raise_for_status()
        departments = list(dict.fromkeys(x[0] for x in response.json()["value"] if x))
    except Exception as exc:
    ...
    for department in departments:
        ...
        for page in range(1, 501):
            ...
            if deadline and time.time() + 35 >= deadline:
                ...
                break
            api = urljoin(TARGET_URL, "/articleCommonController/lists.do")
            try:
                response = session.post(api, data={"page": page, "pagesize": 15, "topic": "",
                                        "rdeptid": RDEPTID, "deptid": department},
                                        headers=API_HEADERS, timeout=30)
                response.raise_for_status()
                data = response.json()["value"]
                items, total = data["list"], int(data["total"])
            except Exception as exc:
