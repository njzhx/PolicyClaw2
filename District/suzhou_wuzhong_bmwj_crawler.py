...
def _extract_content(session, article_url, metrics):
    try:
        ...
        media_only = False
        for selector in ('.infoContent_info', '.TRS_UEDITOR', '.article-content', '#UCAP-CONTENT', '.TRS_Editor', '#zoom', '.Custom_UnionStyle', '.article', '.content'):
...
def scrape_data():
    ...
    try:
        ...
        departments = []
        for link in soup.select('a[href*="wzbmxxgkml_all.shtml"]'):
            query = parse_qs(urlsplit(link["href"]).query)
            values = tuple(query.get(key, [""])[0] for key in ('deptid', 'cxtype', 'pid'))
    ...
    for department in departments:
        ...
        for page in range(1, 501):
            ...
            if deadline and time.time() + 35 >= deadline:
                ...
                break
            params = dict(zip(('deptid', 'cxtype', 'pid'), department))
