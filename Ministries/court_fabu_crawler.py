...
TARGET_URL = "https://www.court.gov.cn/fabu/gengduo/22.html"
...
def scrape_data():
    ...
    try:
        ...
        list_container = soup.find('div', class_='sec_list')
        if not list_container:
            print('[ERROR] 最高人民法院发布爬虫：未找到目标容器 div.sec_list')
            return policies, all_items

        ul_element = list_container.find('ul')
        if not ul_element:
            print('[ERROR] 最高人民法院发布爬虫：未找到列表容器 ul')
            return policies, all_items

        li_elements = ul_element.find_all('li')
        if not li_elements:
            print('[ERROR] 最高人民法院发布爬虫：未找到列表项 li')
            return policies, all_items
            ...
            try:
                ...
                if not article_url.startswith('http'):
                    if article_url.startswith('/'):
                        article_url = "https://www.court.gov.cn" + href
                    else:
                        article_url = "https://www.court.gov.cn/" + href

                date_tag = li.find('i', class_='date')
                pub_at = None
                if date_tag:
                ...
                try:
                    ...
                    content_elem = detail_soup.find('div', class_='txt_txt')
                    if not content_elem:
        ...
        print(f'[OK] 最高人民法院发布爬虫：成功抓取 {len(policies)} 条目标日期窗口数据')
        ...
        print(f'[ERROR] 最高人民法院发布爬虫：抓取失败 - {e}')
        print("----------------------------------------")
...
def save_to_supabase(data_list):
    try:
        ...
        return save_to_policy(data_list, "最高人民法院_发布")
    except Exception:
        return data_list
...
def run():
    try:
        ...
        print(f'[ERROR] 爬虫 最高人民法院发布 运行失败 - {e}')
        print("----------------------------------------")
        return []
