...
TARGET_URL = "https://www.119.gov.cn/zfxxgk/fdzdgknr/tzgg/index.shtml"
...
def scrape_data():
    ...
    try:
        ...
        list_container = soup.find('div', class_='container-list')
        if not list_container:
            print('[ERROR] 国家消防救援局政务公开爬虫：未找到目标容器 div.container-list')
            return policies, all_items

        li_elements = list_container.find_all('li', class_='list-item-text')
        if not li_elements:
            print('[ERROR] 国家消防救援局政务公开爬虫：未找到列表项 li.list-item-text')
            return policies, all_items
            ...
            try:
                ...
                if not article_url.startswith('http'):
                    if article_url.startswith('/'):
                        article_url = "https://www.119.gov.cn" + href
                    else:
                        article_url = "https://www.119.gov.cn/" + href
                ...
                span_tag = li.find('span', class_='time')
                pub_at = None
                if span_tag:
                ...
                if not pub_at:
                    ...
                    if date_match:
                        ...
                        date_match = re.search(r'/(\d{4})/(\d{4,6})\.shtml', href)
                        if date_match:
                            year = date_match.group(1)
                            rest = date_match.group(2)
                            if len(rest) >= 6:
                                try:
                                    pub_at = datetime.strptime(f"{year}-{rest[0:2]}-{rest[2:4]}", '%Y-%m-%d').date()
                                except ValueError:
                                    pass
                ...
                try:
                    ...
                    content_elem = detail_soup.find('div', class_='b-container')
                    if not content_elem:
        ...
        print(f'[OK] 国家消防救援局政务公开爬虫：成功抓取 {len(policies)} 条目标日期窗口数据')
        ...
        print(f'[ERROR] 国家消防救援局政务公开爬虫：抓取失败 - {e}')
        print("----------------------------------------")
...
def save_to_supabase(data_list):
    try:
        ...
        return save_to_policy(data_list, "国家消防救援局_政务公开")
    except Exception:
        return data_list
...
def run():
    try:
        ...
        print(f'[ERROR] 爬虫 国家消防救援局政务公开 运行失败 - {e}')
        print("----------------------------------------")
        return []
